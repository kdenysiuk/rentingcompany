...
CREATE TABLE IF NOT EXISTS `keith_denysiuk`.`ensurance` (
  `id_ensurance` INT NOT NULL AUTO_INCREMENT,
  `e_name` VARCHAR(45) NOT NULL,
  `price` INT NOT NULL,
  PRIMARY KEY (`id_ensurance`));
...
CREATE TABLE IF NOT EXISTS `keith_denysiuk`.`orders` (
  ...
  `ensurance_id_ensurance` INT NOT NULL,
  `building_id_building` INT NOT NULL,
  PRIMARY KEY (`id_order`),
  FOREIGN KEY (`user_id_user`)
  REFERENCES `user` (`id_user`) ON DELETE CASCADE,
  FOREIGN KEY (`ensurance_id_ensurance`)
  REFERENCES `ensurance` (`id_ensurance`) ON DELETE CASCADE,
